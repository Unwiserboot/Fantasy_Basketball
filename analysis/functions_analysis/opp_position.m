...
%Assigne opponent position team rating to each player
    ...
    if strcmp(analysis_players.position(i), 'PF') == 1
        analysis_players.opp_team_position_fp_allowed(i) = analysis_teams{3,2}.total_fp_allowed(row_team_name);

    elseif strcmp(analysis_players.position(i), 'C') == 1
        analysis_players.opp_team_position_fp_allowed(i) = analysis_teams{4,2}.total_fp_allowed(row_team_name);

    elseif strcmp(analysis_players.position(i), 'PG') == 1
        analysis_players.opp_team_position_fp_allowed(i) = analysis_teams{5,2}.total_fp_allowed(row_team_name);
